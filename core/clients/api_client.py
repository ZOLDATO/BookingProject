...
from core.schemas.booking_schemas import BOOKING_SCHEMA
import allure
...
class APIClient:
    ...
    def auth(self):
        with allure.step("Getting authenticate"):
            ...
            response = self.session.get(url, json=payload, timeout=Timeouts.TIMEOUT.value)
            response.raise_for_status()
        with allure.step("Cheking status code"):
            assert response.status_code == 200, f"Expected status 200 but got {response.status_code}"
        token = response.json().get("token")
        with allure.step("Updating header with authorization"):
            self.session.headers.update({"Authorization": f"Beawer {token}"})

    def get_booking_by_id(self, id):
        with allure.step("Getting booking information"):
            url = f"{self.base_url}{Endpoints.BOOKING_ENDPOINT.value}/{id}"
            response = self.session.get(url, timeout=Timeouts.TIMEOUT.value)
            response.raise_for_status()
        with allure.step("Checking status code and json response"):
            assert response.status_code == 200, f"Expected status 200 but got {response.status_code}"
    ...
    def update_booking(self, id, booking_data: dict):
        with allure.step("Getting object with booking"):
            url = f"{self.base_url}{Endpoints.BOOKING_ENDPOINT.value}/{id}"
    ...
    def partial_update_booking(self, id, booking_partial_data: dict):
        with allure.step("Getting object with booking"):
            url = f"{self.base_url}{Endpoints.BOOKING_ENDPOINT.value}/{id}"
            response = self.session.patch(url, json=booking_partial_data)
            response.raise_for_status()
        with allure.step("Checking status code"):
            assert response.status_code == 200, f"Expected status 200 but got {response.status_code}"
        return response.json()
